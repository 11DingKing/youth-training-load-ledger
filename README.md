...
The default bootstrap advisor is `advisor@example.test` with password `change-me-now`. Set `BOOTSTRAP_ADMIN` and `BOOTSTRAP_PASSWORD` in deployed environments. Session tokens are returned only at login; the database stores SHA-256 token digests. Passwords use PBKDF2-SHA256 with per-user random salt.
