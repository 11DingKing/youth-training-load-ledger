...
func (t *Tx) CurrentConsent(ctx context.Context, athleteID int64) (domain.GuardianConsent, error) {
	return currentConsent(ctx, t.tx, athleteID)
}

func (s *Store) CurrentConsent(ctx context.Context, athleteID int64) (domain.GuardianConsent, error) {
	return currentConsent(ctx, s.db, athleteID)
}

func currentConsent(ctx context.Context, exec Executor, athleteID int64) (domain.GuardianConsent, error) {
	var c domain.GuardianConsent
	err := exec.QueryRowContext(ctx, `SELECT id, athlete_id, guardian_id, status, terms_version, effective_at,
...
func (s *Store) UpdateConsent(ctx context.Context, consent domain.GuardianConsent) error {
	return s.WithTx(ctx, func(tx *Tx) error {
		return tx.UpdateConsent(ctx, consent)
	})
}
