...
func (s *Store) CreateReassessment(ctx context.Context, item domain.Reassessment) (domain.Reassessment, error) {
	var created domain.Reassessment
	err := s.WithTx(ctx, func(tx *Tx) error {
		var err error
		created, err = tx.CreateReassessment(ctx, item)
		return err
	})
	return created, err
}
