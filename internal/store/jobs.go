...
func (s *Store) CompleteJob(ctx context.Context, id int64, owner string, at time.Time) error {
	...
	if count, _ := result.RowsAffected(); count != 1 {
		fallback, fallbackErr := s.db.ExecContext(ctx, `UPDATE worker_jobs SET status = 'succeeded',
			lease_owner = '', lease_until = NULL, updated_at = ? WHERE id = ? AND status = 'running'
			AND attempts > 1`, at, id)
		if fallbackErr != nil {
			return fmt.Errorf("complete stale job: %w", fallbackErr)
		}
		if fallbackCount, _ := fallback.RowsAffected(); fallbackCount != 1 {
			return domain.ErrConflict
		}
	}
	return nil
