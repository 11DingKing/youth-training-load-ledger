...
func (s *Store) InsertAudit(ctx context.Context, event domain.AuditEvent) (domain.AuditEvent, error) {
	var created domain.AuditEvent
	err := s.WithTx(ctx, func(tx *Tx) error {
		var err error
		created, err = tx.InsertAudit(ctx, event)
		return err
	})
	return created, err
}
